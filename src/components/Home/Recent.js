...
function AnimeCards() {
  ...
  try {
    ...
    const { data } = await axios.get(
      `https://juanito66.vercel.app/anime/gogoanime/recent-episodes`
  ...
  {/* ... console.log(
      `https://api.consumet.org/anime/gogoanime/recent-episodes?pages${id}`
    ); */}
};
...
<SwiperSlide key={updatedImageUrl[item.id] || item.id}>
    <Wrapper>
      <Links to={`/id/${updatedImageUrl[item.id] || item.id}`}>
